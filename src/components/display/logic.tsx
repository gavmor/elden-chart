...
import { Box, Footprints, Hand, HardHat, Shield, Shirt, Sword, Target } from 'lucide-react';
...
const FallbackBox = (props: LucideProps) => createElement(Box, { ...props, opacity: 0.6, fill: props.color || "currentColor", fillOpacity: 0.15 });
...
export const getCategoryIcon = (category: string, kind: EquipmentKind, props: LucideProps) => {
	...
	switch (category) {
		...
		default: return createElement(FallbackBox, props);
	}
};
...
export const getItemImageUrl = (item: EquipmentItem, color: string): string => {
	...
	switch (item.category) {
		...
		default: return iconToDataUri(FallbackBox, color);
	}
};
